...
public class AttributesHelper {
    ...
    /**
     * Adds attribute value specified by text value. Chooses one from system or creates a new one if such value does not
     * exist.
     * 
     * @param value
     ...
     *             when new group cannot be created because of name conflict (may occur when concurrent modifications
     *             are performed).
        ...
        for (Iterator<DLObject> iter = groupValues.iterator(); iter.hasNext();) {
            ...
            if (element.getValue().equalsIgnoreCase(value)) {
                if (!element.getValue().equals(value)) {
                    logger.warn("Found value in group but with different case. Value: " + element.getValue()
                            + " Group id: " + group.getId() + " Language name: " + selLang);
                }
                return element;
            }
        }
        throw new IdNotFoundException("Could not find value in group. Value: " + value + " Group id: " + group.getId()
